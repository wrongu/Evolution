...
//	public static final double MUSCLE_MULTIPLIER = 5.0;
	...
	public Joint(double rest_angle1, double rest_angle2, PointMass thePoint, Rod a, Rod b) {
		...
		rod0 = a;
		rod1 = b;
		if(rod0.getOtherEnd(point) == null || rod1.getOtherEnd(point) == null) {
			System.out.println("You fucked up on a Joint constructor.");
			System.exit(0);
		}
		restValue0 %= 2*Math.PI;
		restValue1 %= 2*Math.PI;
		if(restValue0 < 0) { restValue0 += 2*Math.PI; }
		if(restValue1 < 0) { restValue1 += 2*Math.PI; }
		if(restValue1 < restValue0) { restValue1 += 2*Math.PI; }
	}
...
//	public double getEPMS() {return ENERGY_PER_MUSCLE_STRENGTH;}
	...
	public PointMass getPoint0() {
		return point;
	}
	
	public PointMass getPoint1() { return rod0.getOtherEnd(point); }
	public PointMass getPoint2() { return rod1.getOtherEnd(point); }
}
